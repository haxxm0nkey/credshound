...
func TestWriteBloodHound(t *testing.T) {
	...
	if len(payload.Graph.Nodes) != 5 {
		t.Fatalf("expected host plus location reuse to produce 5 nodes, got %d: %+v", len(payload.Graph.Nodes), payload.Graph.Nodes)
	}
	if len(payload.Graph.Edges) != 4 {
		t.Fatalf("expected 4 edges, got %d: %+v", len(payload.Graph.Edges), payload.Graph.Edges)
	}
	...
	if !hasBloodHoundNodeKind(payload.Graph.Nodes, bloodHoundKindCreds) {
		t.Fatalf("expected creds node kind, got %+v", payload.Graph.Nodes)
	}
	if hasBloodHoundNodeKind(payload.Graph.Nodes, "Secret") || hasBloodHoundNodeKind(payload.Graph.Nodes, "Creds") {
		t.Fatalf("expected no built-in-looking credential node kind, got %+v", payload.Graph.Nodes)
	}
	if !hasBloodHoundNodeKind(payload.Graph.Nodes, bloodHoundKindObservation) {
		t.Fatalf("expected observation node kind, got %+v", payload.Graph.Nodes)
	}
	if !hasBloodHoundNodeKind(payload.Graph.Nodes, bloodHoundKindProduct) {
		t.Fatalf("expected product node kind, got %+v", payload.Graph.Nodes)
	}
	if !hasBloodHoundEdgeKind(payload.Graph.Edges, bloodHoundEdgeContainsCreds) {
		t.Fatalf("expected ContainsCreds edge, got %+v", payload.Graph.Edges)
	}
	if !hasBloodHoundEdgeKind(payload.Graph.Edges, bloodHoundEdgeMayAuthenticate) {
		t.Fatalf("expected MayAuthenticate edge, got %+v", payload.Graph.Edges)
	}
	if !hasBloodHoundEdgeKind(payload.Graph.Edges, bloodHoundEdgeContainsLocation) {
		t.Fatalf("expected ContainsLocation edge, got %+v", payload.Graph.Edges)
	}
...
func TestBloodHoundInfersUserProfile(t *testing.T) {
	...
	if !hasBloodHoundNodeKind(payload.Graph.Nodes, bloodHoundKindUserProfile) {
		t.Fatalf("expected user profile node, got %+v", payload.Graph.Nodes)
	}
	if !hasBloodHoundNodeProperty(payload.Graph.Nodes, "path", "/Users/alice") {
	...
	if !hasBloodHoundEdgeKind(payload.Graph.Edges, bloodHoundEdgeHasUserProfile) {
		t.Fatalf("expected HasUserProfile edge, got %+v", payload.Graph.Edges)
	}
	if !hasBloodHoundEdgeKind(payload.Graph.Edges, bloodHoundEdgeContainsLocation) {
		t.Fatalf("expected ContainsLocation edge, got %+v", payload.Graph.Edges)
	}
